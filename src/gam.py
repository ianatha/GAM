...
def normalizeEmailAddressOrUID(emailAddressOrUID, noUid=False, checkForCustomerId=False, noLower=False):
  ...
  if (not noUid) and (emailAddressOrUID.find(':') != -1):
    if emailAddressOrUID[:4].lower() == 'uid:':
      return emailAddressOrUID[4:]
    if emailAddressOrUID[:3].lower() == 'id:':
...
def buildGAPIServiceObject(api, act_as, showAuthError=True):
  ...
  try:
    ...
    if showAuthError:
      stderrErrorMsg('User {0}: {1}'.format(GM_Globals[GM_CURRENT_API_USER], str(e[0])))
...
def buildGplusGAPIObject(user):
  userEmail = convertUIDtoEmailAddress(user)
  return (userEmail, buildGAPIServiceObject('plus', userEmail))
      ...
      try:
        if fullDataRequired is not None:
          warnings = callGAPIitems(rep.userUsageReport(), 'get', 'warnings',
                                   ...
                                   date=tryDate, userKey=userKey, customerId=customerId, fields='warnings')
...
def doCreateAdmin():
  ...
  if role[:4].lower() == 'uid:':
    ...
    body['roleId'] = roleid_from_role(role)
  if not body['roleId']:
...
def doCreateDataTransfer():
  ...
  while i < len(sys.argv):
    ...
    i += 2
  body['applicationDataTransfers'] = [{'applicationId': serviceID}]
  for key, value in list(parameters.items()):
    body['applicationDataTransfers'][0].setdefault('applicationTransferParams', [])
    body['applicationDataTransfers'][0]['applicationTransferParams'].append({'key': key, 'value': value})
  result = callGAPI(dt.transfers(), 'insert', body=body, fields='id')['id']
  print('Submitted request id %s to transfer %s from %s to %s' % (result, serviceName, old_owner, new_owner))
...
def getPermissionId(argstr):
  ...
  if permissionId.find('@') == -1:
    permissionId = '%s@%s' % (permissionId, GC_Values[GC_DOMAIN])
    ...
    for fileId in fileIdSelection['fileIds']:
      ...
      for exportFormat in exportFormats:
        ...
        try:
          ...
          sys.stderr.write('Format ({0}) not available\n'.format(extension[1:]))
        if fh:
          closeFile(fh)
          os.remove(filename)
...
def showGplusProfile(users):
  ...
  for user in users:
    ...
    if not gplus:
      continue
    sys.stderr.write('Getting Gplus profile for %s\n' % user)
    try:
      ...
      entityServiceNotApplicableWarning('User', user, i, count)
  sortCSVTitles(['id',], titles)
  writeCSVfile(csvRows, titles, list_type='Gplus Profiles', todrive=todrive)
...
def doDelProjects(login_hint=None):
  ...
  projects = callGAPIpages(crm.projects(), 'list', 'projects')
  for project in projects:
    pid = project['projectId']
    if pid.startswith('gam-project-'):
      print('Deleting %s...' % pid)
      try:
        callGAPI(crm.projects(), 'delete', throw_reasons=[GAPI_FORBIDDEN], projectId=pid)
      except GAPI_forbidden:
        pass

def enableProjectAPIs(simplehttp, httpObj, project_name, checkEnabled):
  _, c = simplehttp.request(GAM_PROJECT_APIS, 'GET')
  apis = c.splitlines()
    ...
    for enabled in enabledServices:
      if 'serviceName' in enabled:
        if enabled['serviceName'] in apis:
          ...
          print(' API %s (non-GAM) is enabled (which is fine)' % enabled['serviceName'])
  for api in apis:
    while True:
...
def doUpdateProject(login_hint=None):
  ...
  try:
    cs_json = json.loads(cs_data)
    project_name = 'project:%s' % cs_json['installed']['project_id']
...
def doCreateProject(login_hint=None):
  ...
  body = {'projectId': project_id, 'name': 'GAM Project'}
  while True:
    create_again = False
...
and accept the Terms of Service (ToS). As soon as you've accepted the ToS popup, you can return here and press enter.''')
  ...
  service_account = callGAPI(iam.projects().serviceAccounts(), 'create',
                             ...
                             body={'accountId': project_id, 'serviceAccount': {'displayName': 'GAM Project'}})
  ...
  console_credentials_url = 'https://console.developers.google.com/apis/credentials?project=%s' % project_id
  while True:
    print('''Please go to:
...
def doUpdateTeamDrive(users):
  ...
  for user in users:
    ...
    if not drive:
      continue
    result = callGAPI(drive.teamdrives(), 'update', body=body, teamDriveId=teamDriveId, fields='id', soft_errors=True)
    if not result:
      continue
...
def doCreateVaultMatter():
  ...
  result = callGAPI(v.matters(), 'create', body=body, fields='matterId')
...
def doCreateVaultHold():
  ...
  callGAPI(v.matters().holds(), 'create', matterId=matterId, body=body)
...
# If any of these characters are present, use the Group Settings API to set the description
  ...
  if gs and not GroupIsAbuseOrPostmaster(body['email']):
    ...
    if settings is not None:
      ...
      callGAPI(gs.groups(), 'update', retry_reasons=['serviceLimit'], groupUniqueId=body['email'], body=settings)
...
def _getBuildingByNameOrId(cd, which_building, minLen=1):
  if not which_building or which_building.lower() == 'id:':
    if minLen == 0:
      return ''
    systemErrorExit(3, 'Building id/name is empty')
  if which_building[:3].lower() == 'id:':
  ...
  if myarg in UPDATE_GROUP_SUBCMDS:
    ...
    if myarg == 'add':
      ...
      if len(users_email) > 1:
        ...
        body = {'role': role, 'email': users_email[0]}
        add_text = ['as %s' % role]
        if delivery:
...
# Compare incoming members and current memebers using the cleaned addresses; we actually add/remove with the original addresses
    ...
    if gs:
      if not GroupIsAbuseOrPostmaster(group):
        ...
        if settings is not None:
          settings.update(gs_body)
          callGAPI(gs.groups(), 'update', retry_reasons=['serviceLimit'], groupUniqueId=group, body=settings)
...
def doUpdateNotification():
  ...
  if get_all:
    notifications = callGAPIpages(cd.notifications(), 'list', 'items', customer=GC_Values[GC_CUSTOMER_ID], fields='items(notificationId,isUnread),nextPageToken')
    for noti in notifications:
      if noti['isUnread'] != isUnread:
        ids.append(noti['notificationId'])
  print('Marking %s notification(s) as %s...' % (len(ids), mark_as))
  for notificationId in ids:
    result = callGAPI(cd.notifications(), 'update', customer=GC_Values[GC_CUSTOMER_ID], notificationId=notificationId, body={'isUnread': isUnread}, fields='notificationId,isUnread')
    if result['isUnread']:
      read_result = 'unread'
    else:
      read_result = 'read'
    print('marked %s as %s' % (result['notificationId'], read_result))
...
def doDeleteNotification():
  ...
  if get_all:
    notifications = callGAPIpages(cd.notifications(), 'list', 'items', customer=GC_Values[GC_CUSTOMER_ID], fields='items(notificationId),nextPageToken')
    for noti in notifications:
      ids.append(noti['notificationId'])
  print('Deleting %s notification(s)...' % len(ids))
  for notificationId in ids:
    callGAPI(cd.notifications(), 'delete', customer=GC_Values[GC_CUSTOMER_ID], notificationId=notificationId)
    print('deleted %s' % id)
...
def doGetNotifications():
  ...
  while i < len(sys.argv):
    ...
    i += 1
  notifications = callGAPIpages(cd.notifications(), 'list', 'items', customer=GC_Values[GC_CUSTOMER_ID])
  for notification in notifications:
    if unread_only and not notification['isUnread']:
      continue
    print('From: %s' % notification['fromAddress'])
    print('Subject: %s' % notification['subject'])
    print('Date: %s' % notification['sendTime'])
    print('ID: %s' % notification['notificationId'])
    print('Read Status: %s' % (['READ', 'UNREAD'][notification['isUnread']]))
    print('')
    print(utils.convertUTF8(utils.dehtml(notification['body'])))
    print('')
    print('--------------')
    print('')
...
OAUTH2_SCOPES = [
  ...
  {'name': 'Directory API - Notifications',
   'subscopes': [],
...
def getScopesFromUser():
  ...
  def _checkMakeScopesList(scopes):
    ...
    for i in range(num_scopes):
      if selected_scopes[i] == '*':
        if not isinstance(OAUTH2_SCOPES[i]['scopes'], list):
          scopes.append(OAUTH2_SCOPES[i]['scopes'])
        else:
          scopes += OAUTH2_SCOPES[i]['scopes']
      elif selected_scopes[i] == 'R':
        scopes.append('%s.readonly' % OAUTH2_SCOPES[i]['scopes'])
      elif selected_scopes[i] == 'A':
        scopes.append('%s.action' % OAUTH2_SCOPES[i]['scopes'])
    if len(scopes) > MAXIMUM_SCOPES:
      return (False, 'ERROR: {0} scopes selected, maximum is {1}, please unselect some.\n'.format(len(scopes), MAXIMUM_SCOPES))
    if len(scopes) == 0:
      return (False, 'ERROR: No scopes selected, please select at least one.\n')
    scopes.insert(0, 'email') # Email Display Scope, always included
    return (True, '')
  ...
  for scope in OAUTH2_SCOPES:
    if scope.get('offByDefault', False):
  ...
  try:
    ...
    if command == 'batch':
      ...
      if argument == 'user':
        ...
        doGetMobileInfo()
      elif argument in ['notifications', 'notification']:
        doGetNotifications()
      elif argument in ['verify', 'verification']:
